...
(defn- execute-transitions [tape transitions head-position current-state blank-symbol trace?]
  (let [read-symbol (if (preds/inside-word? tape head-position)
    ...
    (cond
      ...
      (= head-position (count tape))
        (recur (str tape blank-symbol) transitions head-position current-state blank-symbol trace?)

      (= head-position -1)
        (recur (str blank-symbol tape) transitions 0 current-state blank-symbol trace?)
      ...
      (let [new-tape (write-tape tape head-position (:writes transition))
            newHeadPosition (if (= (:direction transition) :right)
                                (inc head-position)
                                (dec head-position))]
        (if trace? (println (msg/format-transition transition)) ())
        (recur new-tape transitions newHeadPosition (:destination transition) blank-symbol trace?)))))

(defn- get-result [input output end-state verdict]
  {:input input :output output :end-state end-state :verdict verdict})

(defn emulate-tm
  ([tm word] (emulate-tm tm word false))
  ([tm word trace?]
   (assert (= (type tm) -turingmachine) (msg/format-not-expected-type tm "Turingmachine"))
     ...
     (cond
       ...
       :else
       (let [result (execute-transitions word (:transitions tm) 0 (:initial-state tm) (:blank-symbol tm) trace?)]
